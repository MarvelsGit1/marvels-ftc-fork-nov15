...
public class BlueRightAutonomous extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException{
        ...
        robot.camservo.setPosition(0.9);
        ...
        TrajectorySequence conePreload = drive.trajectorySequenceBuilder(startPose)
                ...
                .strafeTo(new Vector2d(-14, 13.5)) //go to middle of field
                .lineTo(new Vector2d(-16.25, 13.5)) //adjust a bit forward to score preload
                ...
                .addTemporalMarker(7.5, () -> robot.liftArm(coneTop, 1)) //drop arm to top of cone stack
                .build();

        //GOTO CONE STACK 5 + PINCH CONE
        TrajectorySequence coneStackFive = drive.trajectorySequenceBuilder(conePreload.end())
                .lineTo(new Vector2d(-63.5, 14.25)) //cone stack pos
                //2.5 seconds
                .waitSeconds(1)
                .addTemporalMarker(0, () -> robot.liftArm(450, 1)) //drop arm to next cone
                .addTemporalMarker(2.5, () -> robot.grabber.setPosition(0.0)) //pinch cone
                .build();

        //GOTO CONE STACK 4 + PINCH CONE
        TrajectorySequence coneStackFour = drive.trajectorySequenceBuilder(conePreload.end())
                .lineTo(new Vector2d(-63.5, 14.25)) //cone stack pos
                //2.5 seconds
                .waitSeconds(1)
                .addTemporalMarker(0, () -> robot.liftArm(300, 1)) //drop arm to next cone
                .addTemporalMarker(2.5, () -> robot.grabber.setPosition(0.0)) //pinch cone
                .build();

        //BACK UP FROM CONE STACK AND SCORE
        TrajectorySequence coneScore = drive.trajectorySequenceBuilder(coneStackFive.end())
                .lineTo(new Vector2d(-16.75, 14.5)) //goto score pos
                .waitSeconds(3)
                .addTemporalMarker(0, () -> robot.liftArm(poleHeight, 1)) //raise arm
                .addTemporalMarker(2, () -> robot.shoulder.setPosition(0.95)) //stretch arm out
                .addTemporalMarker(3, () -> robot.grabber.setPosition(1.0)) //release cone
                .addTemporalMarker(4, () -> robot.shoulder.setPosition(0.6)) //bring arm back
                .addTemporalMarker(6, () -> robot.liftArm(coneTop, 1)) //drop arm to top of cone stack
                .build();

        //PARKING LEFT
        TrajectorySequence parkingOne = drive.trajectorySequenceBuilder(coneScore.end())
                .lineTo(new Vector2d(-62, 14.25))
                .build();

        //PARKING MIDDLE
        TrajectorySequence parkingTwo = drive.trajectorySequenceBuilder(coneScore.end())
                .lineTo(new Vector2d(-40, 13.5))
                .build();

        //PARKING RIGHT
        TrajectorySequence parkingThree = drive.trajectorySequenceBuilder(coneScore.end())
                .lineTo(new Vector2d(-13.5, 13.5))
                .build();
        ...
        drive.followTrajectorySequence(coneStackFive);
        drive.followTrajectorySequence(coneScore);
        if (tagOfInterest != null) {
            if (tagOfInterest.id == RIGHT) {
                drive.followTrajectorySequence(coneStackFour);
                drive.followTrajectorySequence(coneScore);
            }
        }
//        //LOOP CONE PICKUP
